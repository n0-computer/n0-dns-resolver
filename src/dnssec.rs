...
/// Why a DNSSEC-validated lookup was rejected.
///
/// The source of [`crate::Error::Dnssec`]. Validation is fail-closed, so any
/// answer that cannot be proven secure produces one of these reasons.
...
pub enum DnssecError {
    ...
    MissingSignature {},
    /// The RRSIG signer zone is not authoritative for the answer name.
    #[error("RRSIG signer {signer} is not authoritative for {owner}")]
    SignerNotAuthoritative {
        /// The zone the RRSIG claims to be signed by.
        signer: String,
        /// The answer's owner name.
        owner: String,
    },
    /// A wildcard-expanded answer lacks proof that no closer match exists.
    #[error("wildcard-expanded answer for {owner} has no closer-match proof")]
    WildcardUnproven {
        /// The answer's owner name.
        owner: String,
    },
    /// No signed DNSKEY RRset could be fetched for a zone.
    #[error("no signed DNSKEY RRset for zone {zone}")]
    MissingDnskey {
        /// The zone whose DNSKEY RRset could not be obtained.
        zone: String,
    },
    ...
    /// The chain of trust from the root anchors did not validate.
    #[error("chain of trust did not validate")]
    ChainInvalid {},
    /// A NODATA denial (the name exists but has no record of the queried type)
    /// could not be authenticated against the signing zone.
    #[error("NODATA denial could not be authenticated")]
    DenialUnproven {},
    /// The signing zone is nested too deep to validate.
    #[error("signing zone {zone} is too deep ({depth} labels) to validate")]
    ZoneTooDeep {
        /// The zone that exceeded the depth limit.
        zone: String,
        /// The zone's label depth.
        depth: usize,
    },
}
...
/// # Errors
///
/// Returns [`SignatureError::BuildQuery`] if `host` is not a valid DNS name or the
...
/// let _ = id;
/// ```
pub fn build_dnssec_query(host: &str, qtype: TYPE) -> Result<(u16, Vec<u8>), SignatureError> {
    let id: u16 = rand::random();
    let mut packet = Packet::new_query(id);
    packet.set_flags(PacketFlag::RECURSION_DESIRED);

    let name =
        Name::new(host).map_err(|err| e!(SignatureError::BuildQuery, AnyError::from_std(err)))?;
    ...
    *packet.opt_mut() = Some(OPT {
    ...
    let mut bytes = packet
        .build_bytes_vec()
        .map_err(|err| e!(SignatureError::BuildQuery, AnyError::from_std(err)))?;
...
/// # Errors
///
/// - [`SignatureError::KeyTagMismatch`] if the DS key tag does not match the
///   DNSKEY.
/// - [`SignatureError::InvalidKey`] if the DS algorithm does not match the DNSKEY.
/// - [`SignatureError::UnsupportedDigestType`] for an unsupported digest type.
/// - [`SignatureError::DigestMismatch`] if the computed digest differs from the DS.
pub fn verify_ds(owner: &Name<'_>, dnskey: &DNSKEY<'_>, ds: &DS<'_>) -> Result<(), SignatureError> {
    if ds.key_tag != key_tag(dnskey) {
        return Err(e!(SignatureError::KeyTagMismatch));
    }
    if ds.algorithm != dnskey.algorithm {
        return Err(e!(SignatureError::InvalidKey));
    }
    let algorithm = match ds.digest_type {
        1 => &digest::SHA1_FOR_LEGACY_USE_ONLY,
        2 => &digest::SHA256,
        4 => &digest::SHA384,
        digest_type => return Err(e!(SignatureError::UnsupportedDigestType { digest_type })),
    };
    ...
    if computed.as_ref() == ds.digest.as_ref() {
        ...
        Err(e!(SignatureError::DigestMismatch))
    }
}
...
/// # Errors
///
/// Returns a [`SignatureError`] describing which check failed: an empty or
...
/// signature check ([`SignatureError::BadSignature`]).
pub fn verify_rrsig(
    rrsig: &RRSIG<'_>,
    ...
    if !serial_le(rrsig.signature_inception, now) {
        return Err(e!(SignatureError::SignatureNotYetValid));
    }
    if !serial_le(now, rrsig.signature_expiration) {
        return Err(e!(SignatureError::SignatureExpired));
    }
...
fn signed_data(rrsig: &RRSIG<'_>, rrset: &[ResourceRecord<'_>]) -> Result<Vec<u8>, SignatureError> {
    let first = rrset
        .first()
        .ok_or_else(|| e!(SignatureError::EmptyRrset))?;
    ...
    for rr in rrset {
        ...
        {
            return Err(e!(SignatureError::InconsistentRrset));
        }
    }
...
fn canonical_owner(owner: &Name<'_>, rrsig_labels: u8) -> Result<Vec<u8>, SignatureError> {
    ...
    if wanted == count {
        ...
        Err(e!(SignatureError::WildcardLabels))
    }
}
...
/// [`SignatureError::UnsupportedRecordType`].
fn canonical_rdata(rdata: &RData<'_>) -> Result<Vec<u8>, SignatureError> {
    match rdata {
        ...
        other => Err(e!(SignatureError::UnsupportedRecordType {
    ...
    match rrsig.algorithm {
        13 => {
            ...
            key.verify(signed, &rrsig.signature)
                .map_err(|_| e!(SignatureError::BadSignature))
        }
        14 => {
        ...
        algorithm => Err(e!(SignatureError::UnsupportedAlgorithm { algorithm })),
    }
}
...
fn split_rsa_public_key(public_key: &[u8]) -> Result<(&[u8], &[u8]), SignatureError> {
    let (exp_len, rest) = match public_key.split_first() {
        Some((0, rest)) => {
            let (len_bytes, rest) = rest
                .split_at_checked(2)
                .ok_or_else(|| e!(SignatureError::InvalidKey))?;
            (
        ...
        None => return Err(e!(SignatureError::InvalidKey)),
    };
    if exp_len == 0 {
        return Err(e!(SignatureError::InvalidKey));
    }
...
mod tests {
    ...
    fn verify_rrsig_ecdsa_p256_round_trip() {
        ...
        assert!(matches!(
            ...
            Err(SignatureError::KeyTagMismatch { .. })
        ));
    }
    ...
    fn verify_rrsig_rejects_unsupported_algorithm() {
        ...
        assert!(matches!(
            verify_rrsig(&rrsig, &rrset, &dnskey),
            Err(SignatureError::UnsupportedAlgorithm { algorithm: 5, .. })
        ));
    }

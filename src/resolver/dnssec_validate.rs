...
//!
//! [`DnsResolver::validate_answer`] is the entry point the lookup path
...
//! [`crate::Error::Dnssec`]. Three cases lean on the denial-of-existence
...
use crate::{
    Error,
    dnssec::{
        ChainError, DelegatedZone, DenialError, DnssecError, ROOT_TRUST_ANCHORS, ResourceRecord,
        SignedRrset, build_dnssec_query, descend_zone, prove_no_ds, prove_nodata, prove_wildcard,
        trust_root, verify_rrset_with_keys,
    },
};

/// The deepest signing zone the chain walk will fetch keys for.
///
/// [`DnsResolver::trusted_keys_for_zone`] walks every ancestor zone cut of
...
/// Flattens the internal validation error onto the public [`DnssecError`],
/// collapsing the chain-walk and denial-proof detail into a single reason each.
impl From<ValidateError> for DnssecError {
    fn from(err: ValidateError) -> Self {
        match err {
            ValidateError::MissingSignature { .. } => e!(DnssecError::MissingSignature),
            ValidateError::SignerNotAuthoritative { signer, owner, .. } => {
                e!(DnssecError::SignerNotAuthoritative { signer, owner })
            }
            ValidateError::WildcardUnproven { owner, .. } => {
                e!(DnssecError::WildcardUnproven { owner })
            }
            ValidateError::MissingDnskey { zone, .. } => e!(DnssecError::MissingDnskey { zone }),
            ValidateError::Fetch { source, .. } => e!(DnssecError::Fetch { source }),
            ValidateError::Chain { .. } => e!(DnssecError::ChainInvalid),
            ValidateError::MissingSoa { .. }
            | ValidateError::InvalidQname { .. }
            | ValidateError::NonAuthoritativeSoa { .. }
            | ValidateError::DenialUnproven { .. } => e!(DnssecError::DenialUnproven),
            ValidateError::ZoneTooDeep { zone, depth, .. } => {
                e!(DnssecError::ZoneTooDeep { zone, depth })
            }
        }
    }
}
...
impl DnsResolver {
    ...
    pub(super) async fn validate_answer(
        ...
        match self.validate_inner(host, qtype, response).await {
            ...
            Err(err) => {
                ...
                Err(e!(Error::Dnssec { source: err.into() }))
            }
        }
    }
    ...
    /// other outcome is fail-closed and surfaces as [`Error::Dnssec`].
    ...
    async fn fetch_raw(&self, name: &str, qtype: TYPE) -> Result<Vec<u8>, ValidateError> {
        let (_id, query) = build_dnssec_query(name, qtype).map_err(|_| {
            e!(ValidateError::Fetch {
                source: Box::new(e!(Error::InvalidName {
                    name: name.to_string()
                })),
            })
        })?;
        self.send_query(&query).await.map_err(|err| {
            e!(ValidateError::Fetch {
                source: Box::new(err),
            })
        })
...
mod tests {
    ...
    async fn validate_nodata_fails_closed_without_soa() {
        ...
        let result = resolver
            ...
            .await;
        assert!(
            matches!(result, Err(Error::Dnssec { .. })),
